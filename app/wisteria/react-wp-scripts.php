<?php
/**
 * Methods for running React WP Scripts
 */

namespace Wisteria\React;
...
function get_assets_list( string $directory ) {
	...
	if ( is_development() ) {
		error_log( 'manifedt path: ' . var_export( $directory . 'asset-manifest.json', true ) );
		$dev_assets = load_asset_file( $directory . 'asset-manifest.json' );
		error_log( var_export( $dev_assets, true ) );
